...
@export var rise_speed = 1          # rises 2x faster now
@export var rise_every_minutes = 0.5  # how long it waits before rising again
@export var max_height = 120.0         # stops at this Y position
@export var pause_duration = 35.0     # how many seconds it pauses at the top
...
func _ready() -> void:
	...
	_delay_t = start_delay if rise_on_start else 0.0
...
func _process(delta: float) -> void:
	...
	if pausing:
		...
		if pause_timer >= pause_duration:
			pausing = false
			position.y = 0.0   # reset so it can rise again next cycle
			timer = 0.0
